    ...
    <div className="flex h-screen w-full bg-black text-white overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 h-full border-r border-white/10 bg-black/50 backdrop-blur-xl p-6 z-20">
        ...
        <nav className="flex-1 space-y-2">
            <NavItem icon={<Home className="w-5 h-5"/>} label="Discover" active />
        ...
        <div className="pt-6 border-t border-white/10 space-y-2">
            ...
            <div className="flex items-center gap-3 p-3 mt-4 rounded-lg bg-white/5 hover:bg-white/10 transition-colors cursor-pointer">
      ...
      <main className="flex-1 relative h-full w-full">
        ...
        <div className="md:hidden absolute top-0 left-0 w-full p-4 z-20 bg-linear-to-b from-black/80 to-transparent pointer-events-none">
            <h1 className="text-xl font-bold tracking-tighter text-white">Resonance</h1>
        </div>
      </main>
...
function NavItem({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) {
    return (
        <Link href="#" className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-200 ${active ? 'bg-primary/20 text-primary font-semibold' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
            {icon}
            <span>{label}</span>
        </Link>
    )
...
function MobileNavItem({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) {
    return (
        <Link href="#" className={`flex flex-col items-center justify-center w-full h-full gap-1 ${active ? 'text-primary' : 'text-white/50'}`}>
            {icon}
